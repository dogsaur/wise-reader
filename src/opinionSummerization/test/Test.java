...
//		testPreprocess();
		// testLdaModel();
		testWholeProcedure();
//		testSentiment("/Users/chaoqiao/Desktop/p", 1);
	}
	private static void testSentiment(String folder, int type) {		
		
		File root = new File(folder);
		File[] files = root.listFiles();
		int true_cnt = 0;
		for (File file : files) {
			int p_type = classifyDocSentiment(file);
			System.out.print(p_type);
			if (p_type == type) {
				true_cnt += 1;
			}
		}
//		System.out.println("true_cnt:" + true_cnt);
//		System.out.print("total" + files.length);
		System.out.println(true_cnt*1.0/files.length);
	}
	
	private static int classifyDocSentiment(File file) {
		String modelDump1 = "./data/train-results/16-04-27_10：08/iter-1300.json";
